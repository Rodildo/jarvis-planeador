...
**Estado actual (21 de septiembre de 2026):** `MIN_SUPPORTED_BUILD_NUMBER = 2` en el backend, y el APK publicado en el Release de GitHub es exactamente `kAppBuildNumber = 2` (`pubspec.yaml` → `version: 1.0.1+2`). Es la primera vez que se usó el mecanismo de verdad: cualquiera que se haya quedado en la build #1 (la que estrenó el mecanismo, sin este forzado activo todavía) ve la pantalla de actualización obligatoria hasta que instale esta build #2.

Pasos para forzar que todos los usuarios (que ya tengan la build actual o superior) actualicen a una build nueva, la próxima vez que haga falta:
