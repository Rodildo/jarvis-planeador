...
**Superada un día después — ver "Las notas pasaron a sincronizarse con el backend" más abajo.** Se deja esta entrada tal cual quedó escrita porque documenta por qué se tomó la decisión original, no porque siga vigente.
...
## Las notas pasaron a sincronizarse con el backend: la decisión de que fueran "100% locales" tenía un defecto real

23 de septiembre de 2026, un día después de la entrada de arriba. El usuario reportó: *"las notas de texto desaparecen cuando cierras y abres sesión, ¿podrían guardarse en la base de datos y en local en el tlf también?"*

**El defecto:** la entrada anterior documentaba, como decisión consciente, que las notas vivieran solo en `SharedPreferences` — pero el aislamiento de caché por cuenta (ver arriba) hizo explícito un problema que ya existía desde antes: `logout()` limpia la caché local de la cuenta que cierra sesión (correcto para blueprint/historial, que son solo una copia de algo recuperable del servidor). Para notas, que **no tenían ninguna copia en el servidor**, eso significaba borrado real y permanente, sin forma de recuperarlas. La decisión original subestimó esto — "texto del usuario para sí mismo, sin valor en sincronizar" no tuvo en cuenta que "sin sincronizar" también significaba "sin respaldo".

**El arreglo:** tabla `notes` nueva en el backend (`user_id`, `text`, `created_at`, `updated_at`) y cuatro endpoints (`GET/POST/PUT/DELETE /notes`, ver [02-backend.md](02-backend.md)), protegidos por cuenta igual que el resto de la API. El backend pasa a ser la fuente de verdad; la caché en disco del dispositivo (`getCachedNotesFromDisk()`/`cacheNotesLocally()`) se queda, pero cambia de rol — de "el único lugar donde existen" a "copia para que la pantalla cargue al instante", exactamente el mismo papel que ya cumplía para blueprint/historial. Con esto, cerrar sesión (o perder el teléfono, o cambiar de dispositivo) ya no borra nada.

**Lecturas vs. escrituras, tratadas distinto a propósito:**
- **Lectura** (`getNotes()`): tolera un backend inestable — si la llamada de red falla, `notes_screen.dart` sigue mostrando la última copia sincronizada (o reintenta cada 3s si no hay ninguna copia todavía). A diferencia de `getHistory`/`getLifeBlueprint` (que devuelven `[]` en silencio ante cualquier falla), `getNotes()` **lanza una excepción** — una lista vacía es un estado real y válido para una cuenta nueva, así que hace falta poder distinguir "de verdad no hay notas" de "no se pudo saber" para decidir si reintentar o mostrar el estado vacío de una vez.
- **Escritura** (`createNote`/`updateNoteRemote`/`deleteNoteRemote`): exige una respuesta confirmada del servidor antes de tocar la lista en pantalla o la caché en disco — no hay aplicación optimista ni cola de sincronización para cuando vuelva la red. Si la llamada falla, se muestra un error (inline en el diálogo de crear/editar, con `SnackBar` al borrar) y no se aplica el cambio. Esto es deliberado: es el mismo criterio que ya usa el resto de la app para escrituras (blueprint, cuenta) — nunca mostrar algo como guardado cuando no se sabe con certeza que lo está. Se descartó una cola de cambios pendientes offline (aplicar de una vez y reintentar después) por ser una categoría de complejidad nueva (ids temporales, reconciliación, conflictos) que nadie pidió — el pedido era "que no se pierdan", no "que funcione sin conexión".
- El id real de cada nota lo asigna el servidor (`crypto.randomUUID()` en `POST /notes`), no el cliente — evita colisiones si la misma cuenta crea notas desde dos dispositivos.

## Website: GitHub Pages con dominio de IONOS en vez de EasyPanel

23 de septiembre de 2026. El plan original (22 de septiembre) era desplegar `website/` como un servicio "sitio estático" en EasyPanel, igual que el backend. Al pedir el usuario conectarle un dominio propio, se cambió a **GitHub Pages** sobre el mismo repo ya existente (`webjotaelebundle`) en vez de completar ese plan, porque GitHub Pages incluye gratis exactamente lo que hacía falta (hosting estático + HTTPS automático vía Let's Encrypt + soporte de dominio propio con verificación de DNS integrada) sin ocupar un slot de servicio en el VPS del usuario ni depender de configurarlo a mano en EasyPanel. Pasos completos en la sección "Website" de [05-deployment.md](05-deployment.md).

**Elección del dominio:** el usuario tiene 4 dominios comprados en IONOS con el mismo nombre (`jotaelebundle`) y distinta extensión — `.com`, `.es`, `.info`, `.store`. Se recomendó y eligió `.com` por ser la extensión más reconocida y de confianza para la mayoría de visitantes; `.es` habría tenido sentido si el público objetivo fuera solo España, `.info` se asocia más a sitios informativos que a una app, y `.store` a tiendas online — ninguna encajaba tan bien como `.com` para una landing de producto/app.

**`www.jotaelebundle.com` en vez del dominio raíz:** se eligió servir el sitio en el subdominio `www` (registro DNS `CNAME`) en vez del dominio raíz (`jotaelebundle.com` sin `www`), porque un `CNAME` en `www` es más simple de configurar y más robusto ante cambios futuros de IP de GitHub Pages que los registros `A`/`AAAA` que exigiría el dominio raíz (hay que apuntarlos a IPs fijas de GitHub, que en teoría podrían cambiar). El efecto colateral documentado en [05-deployment.md](05-deployment.md) es que quien escriba el dominio sin `www` no llega al sitio a menos que se configure una redirección aparte en IONOS — aceptado a propósito, no un descuido.
