...
- ~~El backend en producción estaba inestable (404 intermitentes en rutas públicas, a veces servía código viejo)~~ — causa real encontrada y resuelta el 22 de septiembre de 2026: **tres contenedores Docker huérfanos** (`jarvis-manual`, `jarvis-manual-backend`, `jarvis-manual.1.f7gf6o64...`), sobras de una prueba manual con `docker-compose up` de hace días nunca limpiada, tenían **la misma regla de Traefik** (`Host(app-jarvisplanner.hzedxy.easypanel.host) && PathPrefix(/api)`) que el contenedor real gestionado por EasyPanel — dos backends compitiendo por el mismo tráfico. Se detuvieron y borraron esos tres contenedores (`docker stop`/`docker rm` en el VPS), confirmado estable con 40/40 peticiones sin fallo durante 80 segundos seguidos (antes fallaba el 100% del tiempo en rachas de más de un minuto). Detalle completo de la investigación en [06-decisions.md](06-decisions.md). La caché local persistente que se agregó como mitigación mientras se investigaba (ver ahí también) se queda — sigue siendo útil independientemente de la salud del backend.
