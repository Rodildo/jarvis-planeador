...
| `/notes` | `NotesScreen` | notas de texto libre; sincronizadas con el backend, con caché local para carga instantánea |
...
- `getTodayLog()` reintenta hasta 3 veces (con una pausa corta entre intentos) ante fallas de red antes de rendirse — la primera llamada tras un arranque en frío puede fallar porque la red del dispositivo aún no está lista, y un solo fallo ahí no debe hacer perder el plan del día. `getLifeBlueprint()`/`getHistory()` en cambio hacen **un solo intento** por llamada — `blueprint_screen.dart`/`history_screen.dart` ya reintentan por su cuenta cada 3 segundos, así que reintentar también aquí adentro solo alargaba cada ciclo sin necesidad. Ver [06-decisions.md](06-decisions.md).
- **De-duplicación de peticiones en curso**: `getLifeBlueprint()`/`getHistory()` comparten un `Future` estático (`_blueprintInFlight`/`_historyInFlight`) mientras hay una llamada en camino — si dos llamadores piden lo mismo casi al mismo tiempo (típicamente la precarga de `ChatProvider` y la pantalla correspondiente abriéndose), la segunda espera el resultado de la primera en vez de disparar un segundo viaje de red redundante.
- **Todas** las llamadas HTTP de este archivo tienen `.timeout(...)` explícito (`_defaultTimeout` = 12s para la mayoría, `_uploadTimeout` = 30s para subir el avatar, `_aiTimeout` = 90s para los 3 endpoints que le pegan a la IA) — ver [06-decisions.md](06-decisions.md) para por qué hacía falta.
- **Caché en memoria de blueprint e historial** (`_cachedBlueprint`, `_cachedHistory`, estáticos, viven lo que dura la sesión de la app, se limpian en `logout()`): `getLifeBlueprint()`/`getHistory()` devuelven la caché si ya existe en vez de golpear la red (aceptan `forceRefresh: true` para saltarla). `submitAssessment()` puebla `_cachedBlueprint` directamente con su propia respuesta (el backend no manda `updatedAt` ahí, así que se sintetiza como "ahora" en UTC) — así la primera vez que se abre "Mi Plan Maestro" justo después de terminar el brief no espera un segundo viaje de red. `ChatProvider` además dispara `getHistory`/`getLifeBlueprint` en segundo plano (sin esperar) **después de que `_hydrateToday()` resuelve** (no en paralelo con ella), para que History y Blueprint ya tengan datos listos cuando el usuario los abra sin competir por ancho de banda con la llamada más importante del arranque — ver [06-decisions.md](06-decisions.md).
- **Caché en disco de blueprint e historial** (`getCachedBlueprintFromDisk()`/`getCachedHistoryFromDisk()`, `SharedPreferences`): capa separada de la caché en memoria de arriba — esta sí sobrevive a cerrar la app. `blueprint_screen.dart`/`history_screen.dart` la leen directo, sin pasar por `getLifeBlueprint()`/`getHistory()`. Se agregó porque el backend puede estar inestable (ver [06-decisions.md](06-decisions.md)) y ninguna cantidad de reintentos del cliente compensa un servidor que a ratos no responde — con esta copia, las pantallas muestran algo al instante incluso si el backend está caído. Se limpia en `logout()` (es de la cuenta, no del dispositivo). Se escribe en `_persistBlueprintDiskCache`/`_persistHistoryDiskCache`, llamadas desde `submitAssessment()` (el único momento en que el plan maestro cambia de verdad) y desde una carga de red exitosa de cada una.
- **Notas** (`notes_screen.dart`): a partir del 23 de septiembre de 2026, el backend (`GET/POST/PUT/DELETE /notes`) es la fuente de verdad — antes vivían solo en `SharedPreferences` y `logout()` las borraba sin que existiera ninguna copia de respaldo (ver [06-decisions.md](06-decisions.md)). `getCachedNotesFromDisk()`/`cacheNotesLocally()` siguen usando el mismo patrón de caché en disco que blueprint/historial, pero ahora es solo eso — una caché para carga instantánea, no el único lugar donde existen. Lecturas (`getNotes()`) toleran un backend inestable devolviendo la última copia sincronizada; a diferencia de `getHistory`/`getLifeBlueprint`, **si la llamada de red falla, lanza una excepción en vez de devolver `[]`** — una lista vacía es un estado real y válido (cuenta nueva), así que `notes_screen.dart` necesita distinguir "de verdad no hay notas" de "no se pudo saber" para decidir si reintenta cada 3s o muestra el estado vacío. Escrituras (`createNote`/`updateNoteRemote`/`deleteNoteRemote`) exigen una respuesta confirmada del servidor antes de aplicar el cambio en pantalla y en la caché — igual que el resto de la app (blueprint, cuenta), nunca se muestra algo como guardado si no se sabe con certeza que lo está.
- **Caché local por cuenta, no por dispositivo**: las tres llaves de arriba (`local_blueprint_cache`, `local_history_cache`, `local_notes`) llevan un sufijo con el `userId` del JWT (`_scopedKey()`, decodifica el payload del token sin verificar firma — no hace falta, solo es una llave local, el backend ya valida el token en cada request). Así, si dos cuentas distintas inician sesión en el mismo teléfono, cada una lee y escribe su propia copia en vez de pisar la de la otra — antes de este cambio (22 de septiembre de 2026) la llave era fija por dispositivo, y la única barrera contra ver datos de la cuenta anterior era que `logout()` la borrara a tiempo (que sí pasaba siempre, pero de forma implícita/frágil, no por diseño). `loadStoredToken()` migra una sola vez, al primer arranque tras esta actualización, cualquier dato que haya quedado bajo la llave vieja sin sufijo hacia la llave nueva con `userId` — para que nadie "pierda" notas ya guardadas. Detalle en [06-decisions.md](06-decisions.md).
...
También nunca muestra "no tienes un blueprint" ni un mensaje de error de red: si no hay nada que mostrar (ni copia en disco ni respuesta de red), la pantalla se queda en el spinner de carga en vez de un estado vacío — mismo criterio aplicado en `history_screen.dart` (nunca "todavía no hay registros", siempre el spinner de `common.loading`) por pedido explícito del usuario: prefiere una carga que nunca termina de resolver a un mensaje que declare "no hay nada". Las dos pantallas primero intentan mostrar la copia en disco al instante (ver la caché en disco más arriba); solo si eso falla (primera vez del usuario, sin nada guardado todavía) caen al viejo camino de red con reintento cada 3 segundos (`Timer.periodic`, cancelado apenas llegan datos o se cierra la pantalla). Ver [06-decisions.md](06-decisions.md) para el detalle completo de esta investigación (timeout explícito, de-duplicación, y finalmente la caché en disco al descubrir que el backend en sí estaba inestable).
