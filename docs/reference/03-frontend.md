...
| `/notes` | `NotesScreen` | notas de texto libre, 100% locales, sin backend |
...
- **Notas locales** (`getNotes()`/`saveNotes()`, `notes_screen.dart`): igual patrón de `SharedPreferences` que el caché de arriba, pero a diferencia de blueprint/historial esto **no es una caché de algo que también vive en el backend** — es el único lugar donde las notas existen, no hay endpoint ni tabla para ellas (texto libre del usuario para sí mismo, no hace falta más). También se limpia en `logout()`.
