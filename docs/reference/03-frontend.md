...
También nunca muestra "no tienes un blueprint" ni un mensaje de error de red: si `getLifeBlueprint()` no trae nada, la pantalla se queda en el spinner de carga en vez de un estado vacío — mismo criterio aplicado en `history_screen.dart` (nunca "todavía no hay registros", siempre el spinner de `common.loading`) por pedido explícito del usuario: prefiere una carga que nunca termina de resolver a un mensaje que declare "no hay nada". Las dos pantallas además reintentan cada 3 segundos (`Timer.periodic`, cancelado apenas llegan datos o se cierra la pantalla) mientras no haya nada que mostrar. Ver [06-decisions.md](06-decisions.md) para el detalle de por qué esto solo no bastaba (faltaba un timeout explícito en `api_service.dart` — ver arriba) y cómo se evita que se vuelva un bucle: un solo intento en vuelo a la vez.
