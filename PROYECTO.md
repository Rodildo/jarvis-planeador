...
- **2026-09-21** — La build con soporte bilingüe (ver entrada de abajo) se compiló, se probó, y se publicó como `kAppBuildNumber = 1` — el APK del Release de GitHub ya es esta versión. Es también la build que **estrena** el mecanismo de versión obsoleta, lo que trae una limitación importante: no hay forma de forzar a nadie con una versión *anterior* a esta a actualizar, porque esas instalaciones no tienen el código que consulta `minBuildNumber` — el mecanismo solo puede forzar actualizaciones entre builds que ya lo incluyen, de aquí en adelante. Detalle completo y pasos para la próxima vez que haga falta forzar una actualización: [docs/reference/06-decisions.md](docs/reference/06-decisions.md) y [docs/reference/05-deployment.md](docs/reference/05-deployment.md).
